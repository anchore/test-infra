...
install: deps venv setup.py requirements.txt ## Install to virtual environment
	$(ACTIVATE_VENV) && pip install deps/anchore-engine
...
push-dev: setup-test-infra ## Push dev Anchore Enterprise Docker image to Docker Hub
	@$(CI_CMD) push-dev-image "$(COMMIT_SHA)" "$(DEV_IMAGE_REPO)" "$(GIT_BRANCH)" "$(TEST_IMAGE_NAME)"

push-rc: setup-test-infra ## (Not available outside of CI) Push RC Anchore Enterprise Docker image to Docker Hub
	@$(CI_CMD) push-rc-image "$(COMMIT_SHA)" "$(DEV_IMAGE_REPO)" "$(GIT_TAG)"

push-prod: setup-test-infra ## (Not available outside of CI) Push release Anchore Enterprise Docker image to Docker Hub
	@$(CI_CMD) push-prod-image-release "$(DEV_IMAGE_REPO)" "$(GIT_BRANCH)" "$(GIT_TAG)"

push-redhat: setup-test-infra ## (Not available outside of CI) Push prod Anchore Enterprise docker image to RedHat Connect
	@$(CI_CMD) push-redhat-image "$(GIT_TAG)"

push-rebuild: setup-test-infra ## (Not available outside of CI) Rebuild and push prod Anchore Enterprise docker image to Docker Hub
