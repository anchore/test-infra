        ...
        if isinstance(e, subprocess.CalledProcessError):
            ...
            if account_override:
                if response == "Unauthorized" or response["httpcode"] == 403:
                    ...
                    logger.error(
                        "account_list | error calling anchore-cli: {0}".format(e)
                    )
        else:
    ...
    try:
        ...
        if isinstance(e, subprocess.CalledProcessError):
            ...
            if response == "Unauthorized" or response["httpcode"] == 403:
                ...
                logger.error(
                    "account_user_list | error calling anchore-cli: {0}".format(e)
                )
        else:
            log_explicit_failure(
                test_type,
...
def account_user_setpassword(context, test_type):
    ...
    try:
        ...
        logger.error(
            "account_user_setpassword | error calling anchore-cli: {0}".format(e)
        )
...
def subscription_deactivate(context, test_type="positive"):
    ...
    try:
        ...
        logger.error(
            "subscription_deactivate | error calling anchore-cli: {0}".format(e)
        )
...
def system_feeds_config_toggle(context, test_type="positive", enable=True):
    ...
    try:
        ...
        logger.error(
            "system_feeds_config_toggle | error calling anchore-cli: {0}".format(e)
        )
